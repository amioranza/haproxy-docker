...
template '/etc/haproxy/haproxy.cfg' do
    ...
    group  'root'
    mode   '0644'
    variables({ :stats_port => node['haproxy']['stats']['port'],
                :stats_user => node['haproxy']['stats']['user'],
                :stats_pass => node['haproxy']['stats']['pass'] })
end
...
docker_container 'haproxy_lb' do
    repo         'haproxy'
    tag          'alpine'
    port         ["80:80", "443:443","#{node['haproxy']['stats']['port']}:#{node['haproxy']['stats']['port']}"]
